...
The simplest case: two conditions, one comparison. You can use either the functional API or the class-based API.
...
# Fit the model using the functional wrapper
...
The simplest case: two conditions, one comparison. You can use the class-based API (`DESeq2`):
...
# Fit the model using the class-based wrapper
...
Specify exactly which comparison you want using the contrast parameter within get_results().

### Two-group contrast

```Python
# Explicit: treated vs control
res = model.get_results(contrast=["condition", "treated", "control"])
```
...
model = DESeq2(counts, metadata, design="~ genotype")
model.run_deseq()

# KO vs WT
res_ko = model.get_results(contrast=["genotype", "KO", "WT"])

# rescue vs KO
res_rescue = model.get_results(contrast=["genotype", "rescue", "KO"])
```
### Discovering available coefficients
...
```Python
# Test for genes with |LFC| > 1 (2-fold change)
res = model.get_results(lfc_threshold=1.0, alpha=0.05)
res.report()
```
...
model = DESeq2(counts, metadata, design="~ batch + condition")
model.run_deseq()

# The condition effect is adjusted for batch
res = model.get_results(contrast=["condition", "treat", "ctrl"])
res.report()
```
...
model = DESeq2(counts, metadata, design="~ genotype + treatment + genotype:treatment")
model.run_deseq()
```
...
model = DESeq2(counts, metadata, design="~ batch + condition")
model.run_deseq()
res = model.get_results(lfc_threshold=1.0)
```
